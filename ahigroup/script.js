document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initializeHomepage() {
        ...
        function setupSlider(propertyList) {
            ...
            doubledList.forEach(ilan => {
                ...
                const imageUrl = anaResim ? `${anaResim}` : 'https://via.placeholder.com/400x250.png?text=Resim+Yok';
    ...
    async function initializeListingsPage() {
        ...
        async function handleSortChange() {
            try {
                ...
                const response = await fetch(`api.php?sortBy=${sortOption}`);
                if (!response.ok) throw new Error('Network response was not ok');
    ...
    async function initializeDetailPage() {
        ...
        const anaResim = ilan.resimler && ilan.resimler.length > 0 ? ilan.resimler[0] : null;
        const anaResimUrl = anaResim ? `${anaResim}` : 'https://via.placeholder.com/800x600.png?text=Resim+Yok';
