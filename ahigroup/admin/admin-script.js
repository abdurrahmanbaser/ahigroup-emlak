document.addEventListener('DOMContentLoaded', () => {
    ...
    const emlakTipiSelect = document.getElementById('emlak_tipi');
    const tumDetayAlanlari = document.querySelectorAll('.detay-alanlari');

    function toggleIlanAlanlari() {
        const secilenTip = emlakTipiSelect.value;

        // Önce tüm özel alanları gizle
        tumDetayAlanlari.forEach(alan => {
            alan.style.display = 'none';
        });

        // Seçime göre ilgili alanları göster
        if (secilenTip.includes('Daire') || secilenTip.includes('İş Yeri')) {
            document.getElementById('konut-alanlari').style.display = 'block';
        }
        if (secilenTip.includes('Arsa')) {
            document.getElementById('arsa-alanlari').style.display = 'block';
        }
        if (secilenTip.includes('Satılık')) {
            document.getElementById('satilik-alanlari').style.display = 'block';
        }
        if (secilenTip.includes('Kiralık')) {
            document.getElementById('kiralik-alanlari').style.display = 'block';
        }
    }

    emlakTipiSelect.addEventListener('change', toggleIlanAlanlari);
        ...
        try {
            ...
            ilanlar.forEach(ilan => {
                ...
                row.querySelector('.delete-btn').addEventListener('click', async () => {
                    ...
                    await fetch(`api.php/${ilan.id}`, { method: 'DELETE' });
     ...
     */
     async function handleFormSubmit(event) {
        ...
        const url = editingId ? `api.php/${editingId}` : 'api.php';
